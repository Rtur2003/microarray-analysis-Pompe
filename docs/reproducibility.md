...
4. Run the analysis (disable network-heavy multiMiR when offline/CI):
   ```bash
   SKIP_MULTIMIR=1 Rscript microarray_analysis_pompe.R
   ```
5. Outputs (CSV, PDF, session info) are written to `results/` by default. Override with `RESULTS_DIR=/path/to/out`.
